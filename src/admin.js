...
export function previewHeroImg() {
  ...
  if (url) {
    ...
    if (realImg) { realImg.classList.add('loading'); realImg.onload = () => { realImg.classList.remove('loading'); }; realImg.src = url; }
  }
}
...
export async function saveHeroBanner() {
  ...
  const url = document.getElementById('hero-media-url')?.value.trim();
  if (url) {
    if (/\.(mp4|webm|mov)(\?|$)/i.test(url)) { bannerState.hero.videoUrl = url; bannerState.hero.img = ''; }
