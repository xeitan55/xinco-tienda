...
export function showAdminSection(section) {
  const map = { dashboard:'dashboard', orders:'orders', products:'products', inventory:'inventory',
    customers:'customers', cupones:'coupons', banners:'cateditor', categorias:'cateditor',
    tracking:'tracking', reportes:'reports', cobranzas:'cobranzas' };
  const view = map[section] || section;
  ['dashboard','orders','products','inventory','customers','coupons','tracking','reports','cateditor','cobranzas'].forEach(v => {
    const el = document.getElementById('admin-section-' + v);
  ...
  if (view === 'inventory') initInventoryChart();
  if (view === 'cobranzas') window.initCobranzasSection?.();
  if (view === 'coupons') showAdminCouponTab?.('active');
}
...
export function editProductAdmin(id) {
  ...
  document.getElementById('ep-colors').value = (p.colors || []).join(', ');
  document.getElementById('ep-tags').value = (p.tags || []).join(', ');
  document.getElementById('ep-img').value = p.img || '';
  document.getElementById('ep-images').value = (p.images || []).map(i => i?.url || i).join('; ');
  document.getElementById('product-modal').classList.remove('hidden');
}

export function closeProductModal() {
  document.getElementById('product-modal')?.classList.add('hidden');
  document.getElementById('edit-product-form')?.removeAttribute('data-edit-id');
}
...
export function renderAdminInventory() {
  ...
  container.innerHTML = state.products.map(p => `
    <tr class="border-b-2 border-outline-variant hover:bg-surface-container-low transition-all">
      <td class="py-3 px-4">
        <img src="${p.img}" alt="${p.name}" loading="lazy" class="w-12 h-14 object-cover border-2 border-primary"/>
      </td>
      ...
      <td class="py-3 px-4">
        <span class="badge ${p.stock > 20 ? 'badge-success' : p.stock > 5 ? 'badge-violet' : 'badge-outline'}">${p.stock}</span>
      </td>
      <td class="py-3 px-4 font-body-md text-sm">${p.cat}</td>
      <td class="py-3 px-4">
        <input type="number" value="${p.stock}" min="0" class="w-20 px-3 py-2 bg-surface-container-low border-2 border-outline-variant text-on-surface font-body-md text-sm" onchange="updateInventoryStock(${p.id}, this.value)"/>
      </td>
    </tr>`).join('');
}
